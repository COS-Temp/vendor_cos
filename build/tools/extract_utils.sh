...
function setup_vendor() {
    ...
    export COS_ROOT="$3"
    if [ ! -d "$COS_ROOT" ]; then
        echo "\$COS_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$COS_ROOT/$OUTDIR" ]; then
        mkdir -p "$COS_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$COS_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$COS_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$COS_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$COS_ROOT"/vendor/cos/build/tools/smali/baksmali.jar
        export SMALIJAR="$COS_ROOT"/vendor/cos/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$COS_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$COS_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$COS_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$COS_ROOT"/vendor/cos/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$COS_ROOT"/"$OUTDIR"/radio
